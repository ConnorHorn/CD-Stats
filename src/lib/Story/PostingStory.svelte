<script>
    ...
    if(archiveData.postCount > 6000){
        specialComment = 'Too many posts. Try this out:<br><a href="https://gprivate.com/6863y" class="text-blue-300">Link</a>';
    }
    ...
    onMount(() => {
        ...
        if(archiveData.postCount > 0){
            typeItInstance = new TypeIt(typeItElement, {
                ...
                .pause(1500)
                .delete()
                .type('You average '+Math.round((archiveData.likeCount/archiveData.postCount)*10)/10+' likes per post.')
                .pause(1500)
                .delete()
                .exec(() => {
