<script>
    ...
    let specialComment = "Probably normal to have one or two people ignored.<br> One could argue "+combinedList.length+" Users is good.";
    if(combinedList.length > 5){
        specialComment = combinedList.length+' people blocked<br>Nothing crazy, but perhaps worthy of retrospection';
    }
    if(combinedList.length > 20){
        specialComment = 'That is a bunch of blocked users<br>'+combinedList.length+'?<br>Maybe you should try to be more open minded.';
    }
    if(combinedList.length > 50){
        specialComment = combinedList.length+' Users Muted or Ignored<br>.<br>.<br>.<br> Look Inwards';
    }
