...
function calcFlags(data) {
    ...
    const sortedMonths = fillMissingDates(monthlyCountMap, toMonthYearString, new Date(firstFlagDate), new Date());
    const sortedYears = fillMissingDates(yearlyCountMap, toYearString, new Date(firstFlagDate), new Date(), true);
    ...
    const firstFlagDateString = firstFlagDate ? `${firstFlagDate.getMonth() + 1}/${firstFlagDate.getDate()}/${firstFlagDate.getFullYear()}` : '';
...
// Initialize likes maps with months and years from January 2019 onwards set to zero
    ...
    const minNonPmDate = nonPmDateStrings.length > 0
